...
const DEMO_URL = "https://demo.pulsescope.wikolabs.com";
...
export default function DemoPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: PAL.bg, color: PAL.txt1, display: "flex", flexDirection: "column" }}>
      ...
      {/* NAV */}
      <nav className="demo-section" style={{ padding: "16px 40px", borderBottom: `1px solid ${PAL.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(10,22,40,0.82)", backdropFilter: "blur(20px)" }}>
        ...
        <div style={{ display: "inline-flex", border: `1px solid ${PAL.border}`, borderRadius: 100, padding: 2, background: PAL.surface }}>
          <button onClick={() => setLang("fr")} style={{ background: lang === "fr" ? PAL.accent : "transparent", color: lang === "fr" ? "#04080F" : PAL.txt2, border: "none", padding: "4px 12px", fontSize: 11, fontWeight: 700, letterSpacing: 0.5, cursor: "pointer", borderRadius: 100, fontFamily: "inherit" }}>FR</button>
          <button onClick={() => setLang("en")} style={{ background: lang === "en" ? PAL.accent : "transparent", color: lang === "en" ? "#04080F" : PAL.txt2, border: "none", padding: "4px 12px", fontSize: 11, fontWeight: 700, letterSpacing: 0.5, cursor: "pointer", borderRadius: 100, fontFamily: "inherit" }}>EN</button>
        </div>
      </nav>
      ...
      <section className="demo-section" style={{ padding: "0 40px 40px", maxWidth: 1280, margin: "0 auto", width: "100%", flex: 1, display: "flex", flexDirection: "column" }}>
        <div className="demo-iframe-wrap" style={{ position: "relative", flex: 1, minHeight: 520, height: "70vh", background: PAL.bg2, border: `1px solid ${PAL.border}`, borderRadius: 18, overflow: "hidden" }}>
          {/* Placeholder canvas while iframe URL doesn't yet exist */}
          ...
          {/* The actual iframe — URL does not yet exist, browser will show its own error */}
        ...
        <div style={{ marginTop: 20, display: "flex", justifyContent: "center" }}>
          <button data-cal-link="wikolabs-team/30min" data-cal-namespace="wk30min" data-cal-config='{"layout":"month_view"}'
            style={{ background: PAL.accent, color: "#04080F", border: "none", borderRadius: 10, padding: "12px 24px", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>
            📅 {t.contact}
          </button>
        </div>
      </section>
